...
def load_data():
    data = "global_terror.csv" #load dataset
    global df    #set to none at last so as to manage memory  
    df = pd.read_csv(data) #read dataset
  ...
  # dropdown -chart
    ...
    dropdown_val = {"Terrorist organisation":'gname',
    ...
    dropdown_values = [{"label":keys, "value":value} for keys, value in dropdown_val.items()]
...
def create_app_ui():
    layout=html.Div([html.H1(
        id='head_1',
        children='Terrorism analysis and Insights',
